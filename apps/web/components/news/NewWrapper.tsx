...
function NewsProvider(props: any) {
  ...
  return (
    <NewContext.Provider
      value={{ headerBackGround, setHeaderBackground, tag, setTag, listTag, setListTag }}
    >
      {props.children}
    </NewContext.Provider>
  );
...
function Content({children = null}: ContentProps) {
  const { headerBackGround, listTag, setListTag, setTag } = useContext(NewContext);
  ...
  return (
    <>
      ...
      {!children ? <div className={c.contents}>
        <div className={c.section}>
          <div className={c.newsCategories}>
            <div onClick={() => handlers.toggle()} className={`${c.curentCategory} ${c["only-sp"]} ${openedTag ? c.open : ""}`}>すべて</div>
            <ul className={openedTag ? c.openList : ""}>
              <li onClick={() => {
                  setTag("")
              }}>
                <Link  href="/jp/news" className={pathName === "/jp/news" ? c.current : ""}>
                すべて
                </Link>
              </li>
              ...
              <li onClick={() => {
                ...
                <Link href="/jp/news/tag/001-プレスリリース"  className={pathName === "/jp/news/tag/001-プレスリリース" ? c.current : ""}>
                プレスリリース
                </Link>
              </li>
              ...
              <li onClick={() => {
                ...
                <Link href="/jp/news/tag/010-ニュース" 
                className={pathName === "/jp/news/tag/010-ニュース" ? c.current : ""}
                >
                ニュース
                </Link>
              </li>
              <li>
                <Link href="/jp/news/tag/020-イベント-セミナー情報"
                  className={pathName === "/jp/news/tag/020-イベント-セミナー情報" ? c.current : ""}>
                イベント・セミナー情報
                </Link>
              </li>
              <li>
                <Link href="/jp/news/tag/030-オンライントレーニング-体験会"  className={pathName === "/jp/news/tag/030-オンライントレーニング-体験会" ? c.current : ""}>
                オンライントレーニング・体験会
                </Link>
              </li>
            </ul>
          ...
          <div className={c.newsIndex}>
            <ul className={c.listedNews}>
              {listTag.length > 0 && listTag.map((tag : any, index:number) => {
                const currentTag = tag.attributes;
                return (
                  <li key={index}>
                  <div className={c.listedNewsHead}>
                    <div className={c.newsDate}>
                   {currentTag.time}
                    </div>
                    <ul  className={c.newsCats}>
                      {currentTag.tag.length > 0 &&  currentTag.tag.map((t:any, i:number) => {
                        return (
                  ...
                  <div className={c.listedNewsTitle}>
                    <Link href={`/jp/news/${currentTag.id_new}`}>{currentTag.title}</Link>
                  </div>
                </li>
                )
              })}
